...
int fast_odes(double t, const double y[], double dydt[],void *Paramstuff)
{
  ...
  double beta_bar = ODEParams->beta_bar;
  double beta_var = ODEParams->beta_var;
  double noise = ODEParams->noise;
  double gam = ODEParams->gam;
  double mu = ODEParams->mu;
  double N0 = ODEParams->N0;

  // ------------------------------------------ ODEs -------------------------------------------- //
  
  //Constant Noise:
  //dydt[0] = mu*N0 - beta_bar*y[0]*y[1]/N0 - noise - mu*y[0]; //dS
  //dydt[1] = beta_bar*y[0]*y[1]/N0 + noise - (mu + gam)*y[1]; //dI
  
  // Random Parameters:
  //dydt[0] = mu*N0 - beta_bar*(1 + noise*(1/sqrt(y[1]))) * y[0]*y[1]/N0 - mu*y[0]; //dS
  //dydt[1] = beta_bar*(1 + noise*(1/sqrt(y[1]))) * y[0]*y[1]/N0 - (mu + gam)*y[1]; //dI
  
  // Random params w/o demography:
  dydt[0] = - beta_bar*(1 + noise*(1/sqrt(y[1]))) * y[0]*y[1]/N0; //dS
  dydt[1] = beta_bar*(1 + noise*(1/sqrt(y[1]))) * y[0]*y[1]/N0 - gam*y[1]; //dI
  return GSL_SUCCESS;
}
